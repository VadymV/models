...
# ==============================================================================
"""
Converts the NICO Vision dataset to TFRecords of TF-Example protos.

This module creates two TFRecord datasets: one for train set
and one for test set. Each TFRecord dataset is comprised of a set of TF-Example
protocol buffers, each of which contains a single image and label.
...
def _get_filenames_and_classes(dataset_dir):
    ...
    """
    icub_root = os.path.join(dataset_dir)
    directories = []
    class_names = []
    for filename in os.listdir(icub_root):
        path = os.path.join(icub_root, filename)
...
def _get_dataset_filename(dataset_dir, split_name, shard_id):
    output_filename = 'nico_%s_%05d-of-%05d.tfrecord' % (
...
def _rename_files(path, basefilename="", depth=6):
        ...
        for file in os.listdir(path):
            ...
            if not os.path.islink(fullpath):
                ...
                if os.path.isdir(fullpath):
                    _rename_files(fullpath, basefilename, depth - 1)
                else:
                    ...
                    if any(ext in extension for ext in _EXTENSIONS):
                        os.rename(fullpath,
                                os.path.dirname(fullpath) + os.path.sep + basefilename + str(
                                    random.randint(100000000000000, 900000000000000)) + extension)
...
def run(training_dataset_dir, testing_dataset_dir, convert_dataset_dir):
    ...
    training_photo_filenames, training_class_names = _get_filenames_and_classes(training_dataset_dir)
    testing_photo_filenames, testing_class_names = _get_filenames_and_classes(testing_dataset_dir)
    ...
    if len(training_class_names) != len(testing_class_names):
        raise ValueError('The training and testing datasets must contain the same classes')
    ...
    labels_to_class_names = dict(zip(range(len(training_class_names)), training_class_names))
    dataset_utils.write_label_file(labels_to_class_names, convert_dataset_dir)

    print('\nFinished converting the NICO Vision dataset!')
